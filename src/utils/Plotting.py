...
class Plot3DBoxBev:
    ...
    def draw_bev(self, dim, loc, rot_y, file_name):
        ...
        # Check if x values are between 300 and 600
        if np.all((pred_corners_2d[:, 0] >= 400) & (pred_corners_2d[:, 0] <= 500) & (pred_corners_2d[:, 1] <= 500)):
            print("!!!!! x values are between 400 and 500 for WARNING", file_name)
            return True
        else:
            return False
            # time.sleep(10)
    ...
    def plot(
        ...
        if class_object in self.object_list:
            ...
            is_warning = self.draw_bev(dim, loc, rot_y, file_name)
        ...
        # draw threshold line
        x4 = np.linspace(400, 400)
        y4 = np.linspace(0, self.shape)
        self.ax2.plot(x4, y4, ls="--", color="red", linewidth=1, alpha=0.5)

        # draw threshold line
        x5 = np.linspace(500, 500)
        y5 = np.linspace(0, self.shape)
        self.ax2.plot(x5, y5, ls="--", color="red", linewidth=1, alpha=0.5)

        x6 = np.linspace(350, 550)
        y6 = np.linspace(500, 500)
        self.ax2.plot(x6, y6, ls="--", color="red", linewidth=1, alpha=0.5)
